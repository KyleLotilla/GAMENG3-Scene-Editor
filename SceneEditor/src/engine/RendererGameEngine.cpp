...
bool RendererGameEngine::init()
{
	...
	this->m_graphicsEngine.compileVertexShader(L"shaders/VertexTextureShader.hlsl", "vsmain", &shader_byte_code, &size_shader);
	...
	this->m_graphicsEngine.compilePixelShader(L"shaders/PixelTextureShader.hlsl", "psmain", &shader_byte_code, &size_shader);
	...
	for (int i = 0; i < 20; i++)
	{
		...
		m_cubes[i]->init(this->m_vertexTextureShader, this->m_pixelTextureShader, this->m_graphicsEngine.getD3DDevice());
		Texture* cubeTexture = new Texture();
		cubeTexture->init(L"assets/textures/brick.png", this->m_graphicsEngine.getD3DDevice());
		this->m_cubes[i]->setTexture(cubeTexture);
		this->m_cubes[i]->setPosition(Vec3(0.0f, 10.0f, 0.0f));
	...
	Texture* cubeTexture = new Texture();
	cubeTexture->init(L"assets/textures/brick.png", this->m_graphicsEngine.getD3DDevice());
	this->m_plane->setTexture(cubeTexture);
	this->m_plane->setScale(Vec3(10.0f, 0.2f, 10.0f));
	PhysicsComponent* cubeComponent = new PhysicsComponent();
	cubeComponent->init(this->m_physicsEngine.getPhysicsCommon(), this->m_physicsEngine.getPhysicsWorld(), this->m_plane);
	cubeComponent->setBodyType(reactphysics3d::BodyType::KINEMATIC);
	this->m_plane->addComponent(cubeComponent);
